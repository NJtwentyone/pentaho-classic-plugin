...
const HvListContainer = React.forwardRef((props, ref) => {
  const {
    id,
    classes: classesProp,
    className,
  ...
  const children = React.useMemo(() => {
    if (!interactive)
      return childrenProp;
    const anySelected = React__default.default.Children.toArray(childrenProp).some(
      (child) => React.isValidElement(child) && child.props.selected && !child.props.disabled
    );
    return React__default.default.Children.map(childrenProp, (child, i) => {
