...
function useInstance(instance) {
  ...
  if (isNoRowsSelected) {
    ...
    if (isAllRowsSelected) {
      ...
      if (isAllPageRowsSelected) {
        ...
        isAllSelectablePageRowsSelected = existsLockedRows && !(page && page.length && page.filter(({ id }) => !lockedSelectionRowIds[id]).some(({ id }) => !selectedRowIds[id]));
        isAllSelectablePageRowsUnselected = !existsLockedRows || !(page && page.length && page.filter(({ id }) => !lockedSelectionRowIds[id]).some(({ id }) => selectedRowIds[id]));
  ...
  const getAutoResetLockedSelectionRows = reactTable.useGetLatest(
    autoResetLockedSelectionRows
  );
  reactTable.useMountedLayoutEffect(() => {
    if (getAutoResetSelectedRows()) {
      dispatch({ type: reactTable.actions.resetSelectedRows });
    }
    if (getAutoResetLockedSelectionRows()) {
      dispatch({ type: reactTable.actions.resetLockedSelectionRows });
    }
  }, [dispatch, data]);
  const toggleAllRowsSelected = React.useCallback(
    (value) => dispatch({ type: reactTable.actions.toggleAllRowsSelected, value }),
    [dispatch]
  );
  const toggleAllPageRowsSelected = React.useCallback(
    (value) => dispatch({ type: reactTable.actions.toggleAllPageRowsSelected, value }),
    [dispatch]
  );
  const toggleRowSelected = React.useCallback(
    (id, value) => dispatch({ type: reactTable.actions.toggleRowSelected, id, value }),
    [dispatch]
  );
  const getInstance = reactTable.useGetLatest(instance);
  const getToggleAllRowsSelectedProps = reactTable.makePropGetter(
    getHooks().getToggleAllRowsSelectedProps,
    {
      instance: getInstance()
    }
  );
  const getToggleAllPageRowsSelectedProps = reactTable.makePropGetter(
    getHooks().getToggleAllPageRowsSelectedProps,
    { instance: getInstance() }
  );
  const toggleRowLockedSelection = React.useCallback(
    (id, value) => dispatch({ type: reactTable.actions.toggleRowLockedSelection, id, value }),
    [dispatch]
  );
...
function prepareRow(row, { instance }) {
  ...
  row.getToggleRowSelectedProps = reactTable.makePropGetter(
