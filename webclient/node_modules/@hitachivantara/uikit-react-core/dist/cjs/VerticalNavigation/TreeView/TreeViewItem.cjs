...
const HvVerticalNavigationTreeViewItem = React.forwardRef(
  (props, ref) => {
    const {
      ...
      disableTooltip,
      ...others
    } = useDefaultProps.useDefaultProps("HvVerticalNavigationTreeViewItem", props);
    ...
    const { index, parentId, level } = descendants.useDescendant(descendant);
      ...
      (event) => {
        if (selectable && !disabled) {
          ...
          if (multiple) {
            if (event.shiftKey) {
              if (selectRange)
                return selectRange(event, { end: nodeId });
            } else if (selectNode)
